...
impl TwoFACodeStore for RedisTwoFACodeStore {
    async fn add_code(
        ...
        let two_fatuple = TwoFATuple(login_attempt_id.as_ref().to_owned(), code.as_ref().to_owned());

        let serialize = serde_json::to_string(&two_fatuple)
        ...
        self
            ...
            .write()
            .await
            .set_ex(&key, serialize, TEN_MINUTES_IN_SECONDS)
            .map_err(|_| TwoFACodeStoreError::UnexpectedError)?;
    ...
    async fn get_code(
        ...
        match self.conn.write().await.get::<_, String>(&key) {
          Ok(value) => {
            let two_fatuple: TwoFATuple = serde_json::from_str(&value)
                .map_err(|_| TwoFACodeStoreError::UnexpectedError)?;

            let login_attempt_id = LoginAttemptId::parse(two_fatuple.0)
                .map_err(|_| TwoFACodeStoreError::UnexpectedError)?;

            let code = TwoFACode::parse(two_fatuple.1)
                .map_err(|_| TwoFACodeStoreError::UnexpectedError)?;
